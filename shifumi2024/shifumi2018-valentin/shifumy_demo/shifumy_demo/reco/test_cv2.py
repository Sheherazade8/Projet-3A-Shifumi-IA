...
index_root = splitted_current_absolute_path.index("shifumy_demo")
PRASAD_PATH = os.path.abspath("/".join(splitted_current_absolute_path[:index_root] + ["third_parties/Classify-HandGesturePose-master"]))
print(PRASAD_PATH)
sys.path.append(PRASAD_PATH)
...
def predict_by_svm(keypoint_coord3d_v, known_finger_poses, svc_file):
	...
	flat_keypoint = np.array([entry for sublist in keypoint_coord3d_v for entry in sublist])
	flat_keypoint = np.expand_dims(flat_keypoint, axis = 0)
	max_index = svc.predict_agent_gesture(flat_keypoint)[0]
...
class webcam():
	...
	def __init__(self, nb_cam = 0):
		self.cap = cv2.VideoCapture(nb_cam)

	def change_to_cam(self, cam):
		self.release()
	...
	def get_table_frames_nb(self, nb_frame):
		frames = []
		for id_frame in range(nb_frame):
			ret, frame = self.cap.read()
			frames.append(frame)
		return frames
	...
	def save_frames(self, nb):
		for i in range(nb):
			frame = self.get_frame()
			path = "/home/kaamelott/Bureau/L1/Stage_Qarma/Git/shifumi2018/resources/data/reco_raw/blbla/opencv" + str(i) + ".jpg"
...
if __name__ == '__main__':
	...
	while True:
		...
		image_v = np.expand_dims((image_raw.astype('float') / 255.0) - 0.5, 0)
		keypoint_coord3d_v = sess.run(keypoint_coord3d_tf, feed_dict = {image_tf: image_v})
		score_label = predict_by_geometry(keypoint_coord3d_v, known_finger_poses, 0.45)
		font = cv2.FONT_HERSHEY_SIMPLEX
		cv2.putText(image_raw, score_label, (10, 200), font, 1.0, (255, 0, 0), 2, cv2.LINE_AA)
		cv2.imshow('Webcam', image_raw)
		if cv2.waitKey(1) & 0xFF == ord('q'):
			break
	wb.release()
