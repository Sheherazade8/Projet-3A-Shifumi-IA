...
def main_loop_terminal():
    ...
    # SAFE: Attendre la fin si les premières images sont reconnues
    gesture_recognition_server.wait_end_of_gesture()
    ...
    while True:
        ...
        print('Faites un geste pour choisir le nombre de rounds:')
        print('- Pierre: {} rounds'.format(nb_round_choice[ROCK]))
        print('- Papier: {} rounds'.format(nb_round_choice[PAPER]))
        print('- Ciseaux: {} rounds'.format(nb_round_choice[SCISSORS]))
        while True:
            if auto_play:
                n_rounds = 40
            else:
                g = gesture_recognition_server.wait_gesture()
                if g == ROCK:
                    n_rounds = nb_round_choice[ROCK]
                elif g == PAPER:
                    n_rounds = nb_round_choice[PAPER]
                elif g == SCISSORS:
                    n_rounds = nb_round_choice[SCISSORS]
                    # n_rounds = input('Nombre de rounds (20 ou 40)?')
                print('n_round = {}'.format(n_rounds))
                gesture_recognition_server.wait_end_of_gesture()
            try:
                n_rounds = int(n_rounds)
                if n_rounds in {20, 40}:
                    break
            except Exception:
...
def main_loop_cv(main_win_size=(1080, 1920), back_win_size=(900, 1440)):
    ...
    try:
        ...
        while True:
            ...
            # Training: Check for all symbol if users is able to do it correctly
            for g_train in [ROCK, PAPER, SCISSORS]:
                while True:
                    for win in (main_window, back_window):
                        s1 = 'Entrainement'
                        s2 = 'Faites le geste {}' \
                            .format(str_rps(g_train, language='FR'))
                        win.show_center_text(s1, s2)
                    g = gesture_recognition_server.wait_gesture(str(g_train))
                    if g == g_train:
                        for win in (main_window, back_window):
                            # SCISSORS is the last gesture in list, end of training after that
                            if g_train == SCISSORS:
                                win.show_center_text("OK, fin de l'entrainement!")
                            else:
                                win.show_center_text(
                                    'OK, essayons un autre geste.')
                        # wait user to remove his hand before continuing
                        gesture_recognition_server.wait_end_of_gesture()
                        break
                    else:
                        s1 = "Oups, j'ai reconnu {}.".format(str_rps(g, language='FR'))
                        s2 = "Recommencez!"
                        for win in (main_window, back_window):
                            win.show_center_text(s1, s2)
                        gesture_recognition_server.wait_end_of_gesture()
            ...
            gesture_recognition_server.wait_gesture()
            for win in (main_window, back_window):
